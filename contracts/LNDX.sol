...
contract LNDX is ERC20 {
    ...
    constructor() ERC20("LNDX LandX Governance Token", "LNDX") {
        ...
        if (block.chainid == 5)
            usdc = 0x4FEB71333c2A9fE81625a5727ab0Ed33dC77B841; // Goerli
    ...
    function unstake(uint256 _amount) external {
        ...
        uint256 usdcBalance = IERC20(usdc).balanceOf(address(this));
        if (usdcBalance == 0) usdcBalance = 1;
        uint256 feeShare = (stakedShares[msg.sender] * totalStaked) /
            usdcBalance; // needs testing
        transfer(msg.sender, _amount);
        IERC20(usdc).transfer(msg.sender, feeShare);
        emit Unstake(msg.sender, _amount, feeShare);
    }

    function calculateShare(address _account) external view returns (uint256) {
        return (stakedBalance[_account] / totalStaked) * 1e12;
    }
