...
contract XBasketBridge is NonblockingLzApp {
    IxBasket public xBasket;
    ...
    constructor(address _lzEndpoint, address _xBasket) NonblockingLzApp(_lzEndpoint) {
        xBasket = IxBasket(_xBasket);
    }
   ...
   function sendToken(uint16 _dstChainId, address _toAddress, uint _amount) public payable {
        if (lzEndpoint.getChainId() == (mainChainId - 100)) {
            xBasket.transferFrom(msg.sender, address(this), _amount);
         } else {
            xBasket.burnFrom(msg.sender, _amount);
         }
         ...
         emit Sent(address(xBasket),  _toAddress, _amount, _dstChainId);
   }
    ...
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory, uint64, bytes memory _payload) internal override {
        (
            ...
            if (lzEndpoint.getChainId() == (mainChainId - 100)) {
                 xBasket.transfer(to, amount);
            } else {
                xBasket.mint(to, amount);
            }
            emit Received(address(xBasket),  to, amount, _srcChainId);
    }
}
